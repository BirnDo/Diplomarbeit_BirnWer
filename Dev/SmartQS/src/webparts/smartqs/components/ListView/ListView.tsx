...
> {
  render() {
    ...
    const listItemComponents = listData.map((item, i) => (
      <ListItem key={`listItem${i}`} render={<ListContent {...item} />} />
    ));
    return (
      <section className="lists">
        <OrderList>{listItemComponents}</OrderList>
      </section>
    );
  }
}
...
class OrderList extends React.Component<OrderListProps, OrderListState> {
  ...
  render() {
    ...
    const children: any = this.getChildrenAsArray();
...
class ListItem extends React.Component<ListItemProps, ListItemState> {
  ...
  componentWillReceiveProps(newProps, newState) {
    if (newProps.swapped || newProps.touched) {
      this.setState({
        offsetY: this.state.offsetY.getBoundingClientRect().top,
      });
    }
  }
  ...
  componentDidUpdate(nextProps) {
    ...
    if (offsetY) {
      const { top } = this.state.offsetY.getBoundingClientRect();
  ...
  transform = (offsetY) => {
    this.state.offsetY.style.transform = `translateY(${offsetY}px)`;
    this.state.offsetY.style.transition = "none";
  };
  // helper function:
  // remove the style attribute from the reference element, reinstating the CSS defined properties.
  resetTransform = () => {
    this.state.offsetY.removeAttribute("style");
  };
  ...
  render() {
    ...
    return (
      <li
        ref={(el) => this.setState({ offsetY: el })}
